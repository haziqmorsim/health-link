...
router.post("/book-appointment", authMiddleware, async (req, res) => {
  try {
    ...
    //Pushing notification to doctor based on userId
    const user = await User.findOne({ _id: req.body.doctorInfo.userId });
    user.unseenNotifications.push({
      type: "new-appointment-request",
      message: `${req.body.userInfo.name} has requested an appointment`,
...
router.post("/check-availability", authMiddleware, async (req, res) => {
  try {
    ...
    if (appointments.length > 0) {
      return res
        .status(200)
        .send({ message: "Appointment slot is not available", success: false });
    } else {
      return res
        .status(200)
        .send({ message: "Appointment slot is available", success: true });
    }
  } catch (error) {
    console.log(error);
    res.status(500).send({
